...
In the report we're going to perform a simple regression analysis using `mtcars`, an R built-in dataset. In Particular we are interested in the following questions: 

> + Is an automatic or manual transmission better for MPG?
> + Quantify the MPG difference between automatic and manual transmissions

The document is structured as follow: after having a look in the dataset, we're going to perform some basic exploratory analysis by computing the correlation matrix and showing a violin plot. In the following section we are going to perform some regression analysis, firstly using a one-variable model (`am`) and secondly adding other predictors to the model. We'll choose the varibles to include in the prevous step by using two different model-selection criteria: *VIF* and *Nested ANOVA* test. At the end we'll check the quality of the model using residual and his derivatives.
...
In addition to the variables `mpg` (consumption) and `am` (transmission type), the most interesting for our purpose are: Displscement [cu.in.] `disp`, horsepower [hp], number of cylinders `cyl`, weight `wt
...
where `am` 0 means automatic and 1 means manual transmission. We want to point out that we decided to treat `cyl` and `carb` as non categorical variable since their values have an intrinsic order and the distance between defferent values as well (for instance 4 cylinders are less then 6 and the difference between 4 and 6 cylinders is in some sense less then the difference between 4 and 8 cylinders).

# Exploratory data analysis

Initially, we want to analize the distribution of the car consumption `mpg` for the two group of `am`. We are using a violin plot because is a combination of the boxplot and an histogram and give us more information in one fell swoop:
...
and we can obsereve that it seems to be a clear distinction for the two groups.
...
where more the dots are `blue`/`red` more the two variable are positevely/negatevely correlated. We've used the `corplot` instead of the `cor` function since that the use of the colors give a better and an immediate understanding of the correlation between couple of variables. 
...
+ `wt`, `disp`, `cyl` and `hp` are highly correlated with `mpg`, which leads us to candidate them as possible predicor variables;
...
The *VIF* quantify the severety of multicolinearity for each predictor. Calculating The VIF for each model defined prevously, we can see the trend of each coeficent as we add new predictors into the model. :
...
The main feature of linear model is to be parsimonious, and for this reason, we are interested in including as less variables as possible.  

## Simple linear regression

Before constructing a multivariable linear regression model, it's a good practice to start with a simple linear regression model and here below is shown some results of the `summary function`: 
...
they should be normal distributed with mean `0` and at first sight they seems to be symmetric respect `0` anyway we'll plot them later.
...
`17.147` and `1.125` are the *estimate* and the *standard error* for the reference level (automatic group);
`7.245` and `1.764` are the *estimate* and the *stardard error* for the increase in mean for the manual group respect to the automatic group (reference level); 

`0.3598` is the percentage of variation explained by the model; the *Adjusted R Squared* takes into account the number of predictor used since the R squared it is a quantity that is going to increase even inserting useless predictor. 
...
`4,902` is the standard error from the fitted line

### R squared and Adjusted R sqaured

```{r}
c(summary(Mtcars.fitSimple)$r.squared,summary(Mtcars.fitSimple)$adj.r.squared)
```

At this stage, since the *p-value* `0.000285` is much less then `0.05`, we could state that the difference in mean `7.245` it's statistically indipendent and it was by pure chance is very low. However we know that omitting other variable to our model can bring us to wrong conclusion.

## Multi-variate linear regression

Using the results made with the model selection methods, we are going to add `wt` and `cyl` into the multivariate regression model:
...
where we can observe how after inserting `wt` and `cyl`into the model, changes the results for `am` beacuse now the difference between automatic and manual ransmission is very little and the `p-value` `0.89334` is no longer statiscal significant.
